...
def main1():
    ...
    for pattern in raw_input:
        ...
        patterns_c.append(
            [
                "".join([x[i] for x in pattern.splitlines()])
                for i in range(len(pattern.splitlines()[0]))
            ]
        )
...
def main2():
    ...
    for pattern in patterns:
        ...
        if c_reflective_i:
            ...
            original_transposed = [
                "".join([x[i] for x in pattern]) for i in range(len(pattern[0]))
            ]
...
def fix_smudge(pattern, original_r_line):
    for r in range(len(pattern)):
        for c in range(len(pattern[0])):
            ...
            if val == ".":
                smudge_corrected[r] = (
                    smudge_corrected[r][:c] + "#" + smudge_corrected[r][c + 1 :]
                )
            else:
                smudge_corrected[r] = (
                    smudge_corrected[r][:c] + "." + smudge_corrected[r][c + 1 :]
            ...
            smudge_transposed = [
                "".join([x[i] for x in smudge_corrected])
                for i in range(len(smudge_corrected[0]))
            ]
...
def find_reflective_i(pattern, orientation, original_sol):
    ...
    potential_reflections = [
        ...
        if [pattern.index(x) for x in pattern][i]
        == [pattern.index(x) for x in pattern][i + 1]
    ]
